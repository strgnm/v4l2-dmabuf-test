...
int main(int argc, char *argv[])
{
	...
	while ((ret = poll(fds, 2, 5000)) > 0) {
		...
		//ret = ioctl(buffer[buf.index].dbuf_fd, DRM_IOCTL_MODE_MAP_DUMB, &arg);
		ret = ioctl(drmfd, DRM_IOCTL_MODE_MAP_DUMB, &arg);
		printf("ret == %d\n", ret);
		if(ret){
			printf("DRM_IOCTL_MODE_MAP_DUMB error!");
			return -1;
		}
		void *map = NULL;
		//map = mmap(0, size, PROT_READ | PROT_WRITE, MAP_SHARED, buffer[buf.index].dbuf_fd, arg.offset);
		map = mmap(0, size, PROT_READ | PROT_WRITE, MAP_SHARED, drmfd, arg.offset);
		if (map == MAP_FAILED){
			printf("mmap failed!\n");
			return -errno;
		}

		process_image(map, size);
		munmap(map, size);
